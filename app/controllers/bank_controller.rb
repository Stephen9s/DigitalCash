class BankController < ApplicationController
  ...
  def withdraw
    ...
    # Get User's total account amount
    account_data = CheckingAccount.find_by_owner_id(current_user.id)
    balance = (account_data.amount).to_i
    
    req_denomination = params[:denomination].to_i
    
    balance_after_denomination_removed = balance - req_denomination
    
    # Check if params[:denomination] is > 0
    if req_denomination > 0 && balance_after_denomination_removed >= 0
      ...
      # Write balance_after_denomination_removed to CheckingAccount
      account_data.update_attribute(:amount, balance_after_denomination_removed)
   ...
   elsif balance_after_denomination_removed < 0
      redirect_to bank_path, :notice => "You don't have enough money!"
   else
      redirect_to bank_path, :notice => "Non-negative currency only!"
   end
  ...
  def deposit
    ...
    if params[:coin]
      ...
      # Retrieve coin denomination
      coin_denomination = Purse.find_by_serial(params[:coin][:serial])
      account_data = CheckingAccount.find_by_owner_id(current_user.id)
      balance = (account_data.amount).to_i
    
      balance_after_sum = (coin_denomination.denomination).to_i + balance
      ...
      if serial_exists
        ...
        # Write balance_after_denomination_removed to CheckingAccount
        account_data.update_attribute(:amount, balance_after_sum)
      end
